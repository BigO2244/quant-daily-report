...
class StrategySpec:
    ...
    exit_rank_multiple: float | None = None
    reduced_allocation: float | None = None
...
def run_backtest(signals: pd.DataFrame, spec: StrategySpec, *, start_date: str, end_date: str) -> dict:
    ...
    for dt in trading_dates:
        ...
        if _should_rebalance(dt, spec.rebalance_mode):
            new_weights, selected = _select_portfolio(daily, spec, active_positions)
        else:
            new_weights = prev_weights.copy()
            selected = list(new_weights.index)

        allocation_scale = _allocation_scale(daily, spec)
        new_weights = new_weights * allocation_scale
...
def _select_portfolio(daily: pd.DataFrame, spec: StrategySpec, active_positions: dict[str, pd.Timestamp]) -> tuple[pd.Series, list[str]]:
    candidates = daily[daily["signal_ready"]].copy()
    selected: list[str] = []

    if spec.selection_mode in {"momentum", "concentration", "slower_rebalance", "regime_gating"}:
    ...
    elif spec.selection_mode == "rank_decay_exit":
        candidates = candidates.sort_values("momentum_score", ascending=False)
        keep = []
        exit_cutoff = int(spec.top_n * (spec.exit_rank_multiple or 2.0))
    ...
    elif spec.selection_mode == "mean_reversion":
        candidates = daily[daily["mean_reversion_signal"]].sort_values("r3", ascending=True)
        selected = _fixed_hold_selection(candidates, spec, active_positions)
    elif spec.selection_mode == "post_move_drift":
        candidates = daily[daily["post_move_drift_signal"]].sort_values("r1", ascending=False)
        selected = _fixed_hold_selection(candidates, spec, active_positions)
    else:
        raise ValueError(f"Unknown selection_mode: {spec.selection_mode}")

    if not selected:
        return pd.Series(dtype=float), []
    weight = 1.0 / len(selected)
    return pd.Series(weight, index=pd.Index(selected, dtype=str), dtype=float), selected


def _fixed_hold_selection(candidates: pd.DataFrame, spec: StrategySpec, active_positions: dict[str, pd.Timestamp]) -> list[str]:
    selected = []
    today = pd.Timestamp(candidates["date"].iloc[0]) if not candidates.empty else None
    for ticker, entry_date in list(active_positions.items()):
        age = (today - entry_date).days if today is not None else 0
        if age < int(spec.fixed_hold_days or 1):
            selected.append(ticker)
    fill = [ticker for ticker in candidates["ticker"].astype(str).tolist() if ticker not in selected]
    return (selected + fill)[: spec.top_n]


def _allocation_scale(daily: pd.DataFrame, spec: StrategySpec) -> float:
    if spec.reduced_allocation is None:
        return 1.0
    if daily.empty:
        return 1.0
    return 1.0 if bool(daily["spy_above_200dma"].iloc[0]) else float(spec.reduced_allocation)
