...
from .indicators import compute_trend_indicators, realized_volatility
...
class Position:
    ...
    """
    def __init__(self, ticker: str, direction: int, shares: int, 
                 entry_price: float, entry_date: pd.Timestamp,
                 entry_atr: float = None, entry_signal: float = None, 
                 sector: str = None):
        self.ticker = ticker
...
class RiskManager:
    ...
    def check_drawdown_circuit_breaker(self, current_equity: float) -> Tuple[bool, float]:
        """
    ...
    def check_exposure_limits(self, positions: Dict[str, Position], 
        ...
        long_exposure = sum(p.shares * p.entry_price for p in positions.values() 
                          if p.direction == 1)
        short_exposure = sum(p.shares * p.entry_price for p in positions.values() 
                           if p.direction == -1)
...
def prepare_data(period: str = "1y", interval: str = "1d") -> pd.DataFrame:
    ...
    signals['trend_dist'] = (signals['close'] - signals['ema_trend']) / signals['ema_trend'] * 100
    ...
    signals['final_signal'] = (0.6 * signals['adx_score'] + 0.4 * signals['trend_score']).clip(0, 100)
...
def calculate_shares(equity: float, price: float, atr: float, 
                     vol_scale: float = 1.0, dd_scale: float = 1.0) -> int:
    """
...
def backtest(signals_df: pd.DataFrame = None) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for i, current_date in enumerate(dates):
        ...
        for ticker, direction, score, entry_atr, sector in pending_entries:
            ...
            shares = calculate_shares(equity, entry_price, entry_atr, vol_scale, dd_scale)
            if shares <= 0:
                continue
    ...
    trades_df = pd.DataFrame(trades) if trades else pd.DataFrame(columns=[
        'ticker', 'direction', 'shares', 'entry_date', 'exit_date',
        'entry_price', 'exit_price', 'pnl', 'hold_days', 'exit_reason', 'sector'
    ])
...
def build_daily_sleeve_output(equity_df: pd.DataFrame, trades_df: pd.DataFrame,
                              sleeve_name: str) -> pd.DataFrame:
    """
    ...
    return df[['date', 'sleeve', 'equity', 'daily_return',
               'gross_exposure', 'net_exposure', 'num_positions']]
...
def compute_stats(equity_df: pd.DataFrame, trades_df: pd.DataFrame,
    ...
    if benchmark_returns is not None and len(benchmark_returns) > 0:
        # Align dates
        aligned = pd.DataFrame({'sleeve': returns, 'benchmark': benchmark_returns}).dropna()
        if len(aligned) > 20:
    ...
    if not trades_df.empty:
        ...
        profit_factor = abs(winners['pnl'].sum() / losers['pnl'].sum()) if losers['pnl'].sum() != 0 else 0
    ...
    if vol_ratio > 1.2:
        ...
        print(f"  ✓  Vol within acceptable range")
