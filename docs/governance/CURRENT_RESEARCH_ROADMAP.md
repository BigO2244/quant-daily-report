...
|----|----------|---------------------------|--------------------------------|----------------|--------------|-----------------|
| FR-050 | Phoenix | Crisis reversal | `fr_archive/fr_050_phoenix_research_spec.md`; Phase C onboarding: `fr_active/fr_069_phoenix_onboarding_packet.md`; evidence template: `fr_active/fr_069_phoenix_evidence_envelope_template.json` | EXTERNAL_DEPENDENCY_BLOCKED — differentiated and risk-shaped, but Phase C liquidity/capacity validation is blocked by Nasdaq Data Link `QELx06`; owner Brett; unblock requires restored Sharadar SEP OHLCV access, OHLCV cache rebuild, PIT liquidity panel, Phoenix Phase C rerun, and Shadow-readiness assessment | `research_registry/research/phoenix.py` + `phoenix_evidence_tracker.py` + `phoenix_phase_b_review.py`; `outputs/research/phoenix_evidence/` | research / external_dependency_blocked |
...
| FR-053 | Argo | Regime allocation overlay / model-selection layer | `fr_archive/fr_053_argo_research_spec.md`; Phase C onboarding: `fr_active/fr_069_argo_onboarding_packet.md`; Phase A framework: `fr_active/fr_069_argo_phase_a_evidence_framework.md` | ACTIVE_RESEARCH — Phase A evidence-consumer framework added; Argo evaluates sleeve evidence for research classifications only and remains non-allocating/non-executing | `research_registry/research/argo.py` + `argo_phase_b_validation.py`; `scripts/research/build_argo_phase_a_evidence_framework.py`; `outputs/research/argo/argo_phase_a_evidence_framework_2026-06-17.json` | research (`meta_model`, `regime_overlay`, `selector`) |
...
| Phoenix | security_selection / crisis_reversal | research / external_dependency_blocked | phoenix.py | run_phoenix_research.py | no | no | research_module / produces_artifacts; blocked on Sharadar SEP OHLCV restoration after Nasdaq Data Link `QELx06` |
...
| Argo | meta_model / regime_overlay | research | argo.py | run_argo_regime_selection.py; build_argo_phase_a_evidence_framework.py | no | no | regime overlay / model-selection layer (FR-053); active selector re-homed from Cassiopeia 2026-06-08; Phase A evidence consumer is research-only |
