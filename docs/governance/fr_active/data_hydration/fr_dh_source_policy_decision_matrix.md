...
| `fundamental_features` | FR-DH-005 / FR-DH-013 | P2 | OBSERVE_ONLY feature JSON | Internal deterministic feature builder | none unless approved | internal | no | n/a | None. | Observe-only feature builder and validator implemented. | Add feature definitions, coverage diagnostics, and sleeve parity before migration. | Security-id resolution and restatement/version policy remain observe-only. |
...
Implement feature coverage/definition diagnostics and P3 observe-only
normalizers, starting with PIT ETF/index membership normalization. Keep approved
env-file or exported vendor keys outside the repository, continue focused
no-secret probes when source coverage changes, and do not wire normalized data
into sleeves until separate migration gates approve consumption.
