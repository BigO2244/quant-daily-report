...
| `macro_regime_features` | Macro regime features | Tier 2 | features | derived | FR-DH feature builders from normalized macro, yield, credit, and VIX artifacts | none unless approved | internal | daily after macro input refresh | after macro input validation | conditional on input release-date policies | feature_date, as_of_date, input_dataset_version | `data/features/macro_regime_features/` | `features.json` | input lineage, release-date policy, macro value coverage | same day as input validation | Argo, Phoenix, risk diagnostics | yield curve flags, credit stress flags, volatility regime flags | Research Data Steward | OBSERVE_ONLY | retire by feature_version with parity report | release-date lag, revised macro history, proxy-source mismatch |
