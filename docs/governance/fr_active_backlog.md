...
3. FR-DH data hydration is the current read-only research-data foundation
   ...
   data-trust summary rendering, blocked-source resolution, and coverage
   hardening before sleeve migration.
...
- **Current state:** FR-DH governance, catalog, hydration swarm, credential-safe
  ...
  feature-store scaffolding, data observability, validation, and internal
  `research_data` load APIs exist. Generated data artifacts remain ignored
  local research outputs. No
...
- **Recommended next implementation:** read-only data-trust summary JSON and
  markdown from the observability manifest. Keep remaining blocked datasets
  classified by source, credential, legal, or business-decision blocker, and
  close restatement/version, release-date, transaction-level, lineage, and
  PIT-safety gaps before any sleeve migration.
