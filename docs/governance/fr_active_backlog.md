...
| FR-069 Research Lab / Modular Sleeve Architecture | Architecture / Design | `DESIGN` | NONE (design-only) | FR-068 PIT foundation, existing sleeve specs (FR-050..053, 067), alpha_lab harness, `docs/governance/caerus_investment_doctrine.md` | primary_active_phase_a_design | Primary active architecture/research workstream. Align the modular sleeve design with the Caerus Investment Doctrine and the portfolio-of-sleeves target state, including 0%-100% sleeve allocation, sleeve onboarding, evaluation, promotion, allocation, and retirement rules. Phase A is architecture/spec/scaffolding only: assign file-area ownership, define registry-first onboarding, document MCP visibility requirements, and identify tests without changing production behavior. Orion/Lyra rationalization belongs inside this architecture rather than as an isolated immediate retirement decision. No production refactor; migration sequenced into future FRs. | Delete the design doc; nothing is wired. No code, execution, registry, or strategy behavior in scope. |
