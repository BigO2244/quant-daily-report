...
| FR-DH | Research Data / Hydration Governance | `DRAFT_RESEARCH` | NONE (read-only research data) | FR-DH index, FR-DH-013 catalog, hydration swarm, P1 normalization, FR-068 PIT foundation, FR-069 sleeve architecture, existing governance registry/backlog/roadmap | read_only_foundation | Data Hydration Package at `docs/governance/fr_active/data_hydration/fr_dh_000_data_hydration_index.md` defines and implements the read-only foundation for canonical research data: catalog, source policy, credential-safe source probes, raw/normalized/feature/manifest layout, freshness monitor, P1 observe-only normalization, validators, `research_data` API, sleeve migration planning, and dashboard/email visibility planning. No live hydration promotion, execution, broker, scheduler, allocation, strategy, sleeve-consumer, or promotion behavior is implemented. | Delete the FR-DH docs, `research_data/`, `scripts/data_hydration/`, `Tests/test_data_hydration_catalog.py`, `Tests/test_data_hydration_swarm.py`, `Tests/test_data_hydration_p1_normalization.py`, and data template/placeholders. Generated data artifacts are gitignored. Runtime behavior is unchanged. |
...
- **Current state:** FR-DH governance, catalog, hydration swarm, credential-safe
  Sharadar probes, dataset freshness, P1 observe-only normalization, P1
  validation, and internal `research_data` load APIs exist. Generated data
  artifacts remain ignored local research outputs. No sleeve consumes these
  artifacts yet. Sharadar may be a source feeding canonical datasets, but it is
