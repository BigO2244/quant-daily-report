...
| FR-069 Research Lab / Modular Sleeve Architecture | Architecture / Design | `PHASE_B_IMPLEMENTED_RESEARCH_ONLY` | NONE (research-only scaffolding) | FR-068 PIT foundation, existing sleeve specs (FR-050..053, 067), alpha_lab harness, `docs/governance/caerus_investment_doctrine.md` | phase_b2_scaffolded | Next major architecture/research workstream after FR-070 observation. Phase B research-only scaffolding is documented in `docs/governance/fr_active/fr_069_phase_b_scaffolding.md` and implemented through `research_registry/sleeves/manifest.json`, the manifest validator, read-only MCP `fr069_sleeve_inventory`, and the Phase B2 static evidence-envelope validator. No production refactor; no runtime strategy-registry behavior change; Orion/Lyra continue evaluation; FR-063 remains active supporting evidence, not retired. Phase C requires separate owner approval even after manifest, validator, evidence-envelope validator, and MCP inventory tests pass. | Revert the Phase B scaffold commit or delete the research-only manifest/evidence/docs/tests/tool registration. No execution, broker, allocation, portfolio construction, model, strategy, cron, live-capital, or secret behavior is in scope. |
