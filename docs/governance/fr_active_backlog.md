...
| FR-DH | Research Data / Hydration Governance | `DRAFT_RESEARCH` | NONE (read-only research data) | FR-DH index, FR-DH-013 catalog, hydration swarm, P1/P2/P3 normalization, feature-store scaffold, FR-068 PIT foundation, FR-069 sleeve architecture, existing governance registry/backlog/roadmap | read_only_foundation | Data Hydration Package at `docs/governance/fr_active/data_hydration/fr_dh_000_data_hydration_index.md` defines and implements the read-only foundation for canonical research data: catalog, source policy, credential-safe source probes, raw/normalized/feature/manifest layout, freshness monitor, P1/P2/P3 observe-only normalization, feature-store scaffolding, validators, `research_data` API, data observability manifest, data-trust summary, sleeve migration planning, and dashboard/email visibility planning. No live hydration promotion, execution, broker, scheduler, allocation, strategy, sleeve-consumer, or promotion behavior is implemented. | Delete the FR-DH docs, `research_data/`, `scripts/data_hydration/`, `Tests/test_data_hydration_catalog.py`, `Tests/test_data_hydration_swarm.py`, `Tests/test_data_hydration_p1_normalization.py`, `Tests/test_data_hydration_p2_normalization.py`, `Tests/test_data_hydration_p3_normalization.py`, `Tests/test_data_hydration_feature_store.py`, `Tests/test_data_hydration_observability.py`, `Tests/test_data_trust_summary.py`, and data template/placeholders. Generated data artifacts are gitignored. Runtime behavior is unchanged. |
...
| FR-085 Shadow Scorecard Publication Integrity Gates | Reporting Integrity / Governance | `PROMOTION_READY` | LOW | `scripts/send_shadow_cio_report.py`, `shadow_nav_series.csv`, `shadow_evaluation.json`, price-hydration `status.json`; provenance context FR-024/FR-066; upstream P2 NAV refresh-freeze follow-up | local_validation_complete_pr_open | Two publication gates added so the Shadow CIO scorecard cannot publish a leaderboard from stale or internally inconsistent research artifacts. Freshness gate withholds rankings/leader/promotion/CIO-takeaway on failed shadow refresh, NAV age beyond tolerance, or corrupt NAV integrity; internal-consistency gate excludes any sleeve from ranking/leader unless its period return is NAV-derived and `valid_day_count >= 10`, emitting a `MODEL SCORECARD: PUBLICATION WITHHELD` block while preserving DATA HEALTH diagnostics. Implemented on branch `fix/scorecard-publication-gates`, draft PR #117 (commit `2ee5cd5`); VM `--dry-run` now withholds with `NAV_STALE` (latest valid NAV 2026-06-05). Not deployed; will withhold daily until the P2 refresh-freeze remediation restores NAV advancement. | Revert PR #117 / commit `2ee5cd5`; scorecard reverts to prior publish-on-stale behavior. No execution, broker, allocation, cron, or price-loading behavior changes either way. |
| FR-086 Shadow Refresh Alpha Inception Handling & NAV Restatement | Research Reporting Integrity / NAV Lineage | `PROMOTION_READY` | MEDIUM | `scripts/refresh_shadow_scorecard_artifacts.py`, shadow definition/performance generation, recovery staging artifact, FR-085 consumer gates, 2026-06-14 NAV restatement | code_promotion_ready_vm_artifact_restated | Root-caused the NAV refresh freeze (stuck at 2026-06-05) to a fail-closed continuity mismatch against mixed/legacy-scale NAV history (`caerus_polaris` prior NAV 38.22 vs operational-scale 1.89), not scheduler/cache/signal. Adds date-aware alpha inception handling (`observation_start_date` 2026-06-23, blank pre-inception cells, first alpha row from `previous_nav=1.0`) plus regression coverage; VM active NAV restated to 29 rows (2026-05-12 → 2026-06-23, SHA `edda4b89…622e4d`). With FR-085 the scorecard un-withholds (Fresh; Leader Orion). Branch `fix/shadow-refresh-freeze`, draft PR #118 (commit `7529c95`); not deployed. | Revert PR #118 / commit `7529c95` for code; restore the preserved pre-repair active NAV file for the artifact; preserve recovery/incident artifacts. |
...
3. FR-DH data hydration is the current read-only research-data foundation
   ...
   blocked-source resolution, dataset-specific coverage hardening, and isolated
   dashboard/email visibility before sleeve migration.
...
6. FR-063 remains active supporting differentiation evidence and must use
   sufficient canonical new-series history before retirement conclusions.
7. Phoenix is no longer externally blocked by Nasdaq Data Link `QELx06`;
...
## FR-DH Data Hydration Governance Package

- **FR category:** FR-DH
- **Title:** Data Hydration Governance Package
- **Status:** `DRAFT_RESEARCH` / read-only implementation
- **Canonical index:** `docs/governance/fr_active/data_hydration/fr_dh_000_data_hydration_index.md`
- **Scope:** Read-only foundation for canonical research data hydration: source
  discovery, raw/normalized/feature/manifest layers, PIT safety, the canonical
  research data catalog, freshness monitoring, model-facing APIs, sleeve
  migration planning, and read-only data trust visibility.
- **Non-goals:** No production trading change, execution logic change, live or
  paper broker submission, live hydration, paid vendor dependency, scheduler
  mutation, model behavior change, allocation change, or promotion action.
- **Current state:** FR-DH governance, catalog, hydration swarm, credential-safe
  ...
  feature-store scaffolding, data observability, data-trust summaries,
  validation, and internal `research_data` load APIs exist. Generated data
  artifacts remain ignored local research outputs. No
  ...
  not call vendors directly; they should use the internal `research_data`
  interface backed by canonical artifacts.
- **Recommended next implementation:** blocked-source resolution and
  dataset-specific coverage hardening, then an isolated dashboard/email
  visibility patch. Keep remaining blocked datasets classified by source,
  credential, legal, or business-decision blocker, and close
  restatement/version, release-date, transaction-level, lineage, and PIT-safety
  gaps before any sleeve migration.
...
- **Validation plan:** `pytest Tests/test_shadow_cio_report.py` (14 passed), `pytest Tests/test_shadow_scorecard_refresh_continuity.py Tests/test_shadow_scorecard_health.py` (16 passed), `py_compile`, `git diff --check`. VM `python -m scripts.send_shadow_cio_report --dry-run` against real `outputs/` now emits `MODEL SCORECARD: PUBLICATION WITHHELD` (`NAV_STALE`, latest valid NAV `2026-06-05`). Full suite: 2256 passed / 11 skipped / 28 failed; the 28 failures are outside the touched scorecard files and must be confirmed pre-existing on `main` before merge.
- **Risks / assumptions:** While the upstream NAV series remains frozen at 2026-06-05, the scorecard will withhold publication daily — this is the intended fail-safe state, not a regression. The 28 unrelated suite failures must be baseline-confirmed against `main` before merge. The internal-consistency gate reuses the existing `valid_days < 10` NOT_READY threshold; if that threshold changes, both code paths should move together.
- **Follow-up (P2 — NAV refresh freeze):** Separate remediation to determine why `trade_date_has_data` returned false for 2026-06-23 (refresh `RuntimeError`, `allow_download=False`) and to backfill the 2026-06-06 → 2026-06-23 gap through the sanctioned restatement workflow (`_append_nav_series` blocks restatement; `_validate_nav_append_continuity` must pass). Now tracked as **FR-086** (branch `fix/shadow-refresh-freeze`, draft PR #118); the confirmed root cause was a NAV-continuity mismatch against mixed/legacy-scale history rather than the hypotheses above. Do not weaken `trade_date_has_data`, `allow_download`, or continuity validation as a shortcut.
- **Implementing branch / PR:** `fix/scorecard-publication-gates`, draft PR #117, commit `2ee5cd5`.
...
## FR-086 Shadow Refresh Alpha Inception Handling & NAV Restatement

- **FR number:** FR-086
- **Title:** Shadow Refresh Alpha Inception Handling & NAV Restatement
- **Date started:** 2026-06-24
- **Phase:** Research Reporting Integrity / NAV Lineage
- **Status:** `PROMOTION_READY` (code); the one-time VM NAV restatement is already applied as an operational recovery action
- **Blast Radius:** MEDIUM (mutates the canonical operational Shadow NAV file via restatement; research-reporting only — no trading/execution/broker/scheduler/price-download change)
- **Dependencies:** `scripts/refresh_shadow_scorecard_artifacts.py` (`_append_nav_series` / `_validate_nav_append_continuity`), shadow definition/performance generation, recovery staging artifact `outputs/recovery_staging/shadow_nav_same_day_20260613T2101Z`; FR-085 (consumer gates); the 2026-06-14 Shadow NAV Observation Restatement; FR-066 / FR-024 provenance.
- **Problem statement:** Shadow NAV stopped advancing after 2026-06-05. Cron was firing, cache/signals were healthy through 2026-06-23, and `trade_date_has_data(..., 2026-06-23)` was true. The refresh fail-closed in NAV append continuity: `SHADOW_NAV_CONTINUITY_MISMATCH: caerus_polaris previous_nav=1.8922922055 does not match prior CSV NAV=38.22213439883912 from 2026-06-05`. The active NAV file carried mixed/legacy-scale history, so replaying a valid operational (same-day close-to-close) NAV row failed validation. Separately, the alpha concentration variants (Polaris_Alpha, Orion_Alpha) activated 2026-06-23 had no inception handling, leaving pre-inception NAV cells ill-defined.
- **Scope (code — PR #118, commit `7529c95`):** (a) add alpha sleeve `observation_start_date: 2026-06-23`; (b) make shadow definition/performance generation date-aware; (c) keep pre-inception alpha NAV cells blank and allow the first real alpha row from `previous_nav=1.0`; (d) add regression coverage.
- **Scope (operational VM recovery — one-time):** re-seeded the active NAV from the clean validated same-day operational staging artifact `recovery_staging/shadow_nav_same_day_20260613T2101Z`; replayed backfill in order 2026-06-15, 06-16, 06-17, 06-18, 06-22, 06-23 (2026-06-19 Juneteenth and the weekend correctly skipped). Final active NAV = 29 rows, 2026-05-12 → 2026-06-23; SHA `edda4b8945bada28ee4aa402f4735f2d54d1fdb62309e612a744811ead622e4d`; alpha sleeves first appear 2026-06-23 (Polaris_Alpha `0.9343324977`, Orion_Alpha `0.9332767749`).
- **Non-goals:** No trading, allocation, execution, broker, scheduler, or price-download logic change. Does not weaken `_validate_nav_append_continuity`, `trade_date_has_data`, or `allow_download`. Should not alter established-sleeve (Polaris/Orion/Lyra) historical NAV beyond the new alpha-inception handling.
- **Planned artifacts:** No new output files. Restated active `shadow_nav_series.csv` (29 rows) and date-aware alpha NAV cells.
- **Validation plan:** Local focused tests 19 passed; broader shadow/restate tests 24 passed; `py_compile` and `git diff --check` passed; VM focused tests 19 passed; VM scratch backfill and active clean-seed backfill exit 0. Combined with FR-085 (PR #117), VM `python -m scripts.send_shadow_cio_report --dry-run` now un-withholds: Data health `Fresh`, Leader `Orion` `+30.25% Since Observation Inception`, window 2026-05-12 → 2026-06-23, Orion absolute NAV `2.0948920242` (~+109.49% vs base 1.0), alpha daily/since-inception ~-6.57% / -6.67%.
- **Risks / assumptions / open items:** (1) The active NAV file was overwritten — confirm the pre-repair legacy-scale file was preserved for rollback/audit per `AGENTS.md`. (2) Root-cause why legacy-scale history persisted despite the 2026-06-14 restatement so it cannot recur on the next append. (3) Confirm the date-aware generation leaves established-sleeve historical NAV byte-identical (golden-NAV diff). (4) Reporting-anchor divergence: the scorecard reports `Since Observation Inception` (2026-05-12 window) while the precompute reports absolute since-inception, so the two surfaces can name different leaders (scorecard: Orion; precompute: Lyra by absolute) — choose a canonical promotion window and label both surfaces explicitly.
- **Implementing branch / PR:** `fix/shadow-refresh-freeze`, draft PR #118, commit `7529c95`.
- **Rollback Reference:** Revert PR #118 / commit `7529c95` for the code; for the artifact, restore the preserved pre-repair active NAV file; do not delete the recovery staging artifact or incident evidence.
