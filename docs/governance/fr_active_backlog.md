...
| FR-105 Global Portfolio Optimizer and Decision Provenance | Portfolio Research / Decision Provenance | `ACTIVE_RESEARCH` | LOW | FR-068 PIT universe, FR-069 research lab, FR-074 candidate lifecycle, FR-DH canonical research data | research_only_opened | Research-only initiative opened after the 2026-06-25 execution transparency audit. It studies global optimizer construction, optimizer-derived holding count, rank-11-to-30 marginal contribution, bottom-quartile holdings, opportunity cost of blocked/clipped trades, and MCP/reporting provenance. No production trading behavior, allocation, sizing, sleeve construction, risk policy, broker submission, scheduler, or promotion behavior is changed or authorized. | Delete `docs/governance/fr_active/fr_105_global_portfolio_optimizer_and_decision_provenance.md` and remove registry/backlog/roadmap rows. Runtime behavior is unchanged. |
...
3. FR-105 global portfolio optimizer and decision provenance is the next
   governed portfolio-construction research lane. It should remain research-only
   until PIT current-policy replay, global top-N frontiers, optimizer-derived
   holding-count variants, and provenance surfaces pass review.
4. FR-DH data hydration is the current read-only research-data foundation
...
5. FR-070 implementation work only if diagnostics produce a classified failure.
6. Orion/Lyra continued evaluation as part of sleeve architecture, not as a
   standalone immediate retirement decision.
7. FR-063 remains active supporting differentiation evidence and must use
   sufficient canonical new-series history before retirement conclusions.
8. Phoenix is no longer externally blocked by Nasdaq Data Link `QELx06`;
