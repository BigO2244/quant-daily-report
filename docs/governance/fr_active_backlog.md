...
| FR-DH | Research Data / Hydration Governance | `DRAFT_RESEARCH` | NONE (read-only research data) | FR-DH index, FR-DH-013 catalog, hydration swarm, P1/P2 normalization, feature-store scaffold, FR-068 PIT foundation, FR-069 sleeve architecture, existing governance registry/backlog/roadmap | read_only_foundation | Data Hydration Package at `docs/governance/fr_active/data_hydration/fr_dh_000_data_hydration_index.md` defines and implements the read-only foundation for canonical research data: catalog, source policy, credential-safe source probes, raw/normalized/feature/manifest layout, freshness monitor, P1/P2 observe-only normalization, feature-store scaffolding, validators, `research_data` API, sleeve migration planning, and dashboard/email visibility planning. No live hydration promotion, execution, broker, scheduler, allocation, strategy, sleeve-consumer, or promotion behavior is implemented. | Delete the FR-DH docs, `research_data/`, `scripts/data_hydration/`, `Tests/test_data_hydration_catalog.py`, `Tests/test_data_hydration_swarm.py`, `Tests/test_data_hydration_p1_normalization.py`, `Tests/test_data_hydration_p2_normalization.py`, `Tests/test_data_hydration_feature_store.py`, and data template/placeholders. Generated data artifacts are gitignored. Runtime behavior is unchanged. |
...
3. FR-DH data hydration is the current read-only research-data foundation
   ...
   feature coverage diagnostics and P3 normalizers before sleeve migration.
...
- **Current state:** FR-DH governance, catalog, hydration swarm, credential-safe
  Sharadar probes, dataset freshness, P1/P2 observe-only normalization,
...
- **Recommended next implementation:** observe-only feature definitions,
  feature coverage diagnostics, and P3 normalizers, starting with PIT ETF/index
  membership normalization. Close restatement/version, release-date,
  transaction-level, lineage, and PIT-safety gaps before any sleeve migration.
