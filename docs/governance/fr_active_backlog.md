...
| FR-005 self-heal recovery integrity | Wave 3 | `DEPLOYED_OBSERVING` | HIGH | Wave 3 deployment | observing | Execution continues only after full bundle validation. Any future execution-path deployment requires operator approval during a scheduled maintenance window outside active market hours. | Revert FR-005 commit; preserve recovery artifacts. |
...
| FR-021 partial execution state normalization | Phase 4 | `BACKLOG` | HIGH | FR-015, FR-017 | not_started | Execution-adjacent semantic work; defer until lower-risk telemetry is established. Any future implementation requires operator approval during a scheduled maintenance window outside active market hours. | Leave current partial-failure interpretation unchanged. |
...
| FR-034 post-submit cash drift reconciliation review | Execution Accounting Review | `READY` | LOW | FR-031, latest paper execution artifacts | ready_for_audit | Determine whether `cash_target_drift` clears after fills/reconciliation, represents expected pending-fill drift, or indicates accounting/reconciliation mismatch. Any resulting runtime change requires operator approval during a scheduled maintenance window outside active market hours. | Docs-only/audit rollback; no runtime behavior should change. |
