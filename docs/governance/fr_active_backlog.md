...
| FR-050 Phoenix Phase B Historical Behavior Review | Investment Confidence / Research Evidence | `ACTIVE_RESEARCH` | LOW | Existing Phoenix research artifacts, VIX/regime data, price panels, shadow snapshots, FR-069 Phase C onboarding | phase_c_research_onboarded | Evaluate Phoenix historical activation behavior, candidate families, regime triggers, overlap versus Polaris/Orion/Lyra, and drawdown/recovery tradeoffs without tuning thresholds. Phoenix is now represented as a governed FR-069 Research-stage sleeve candidate via `docs/governance/fr_active/fr_069_phoenix_onboarding_packet.md` and evidence template `docs/governance/fr_active/fr_069_phoenix_evidence_envelope_template.json`; this does not activate Shadow or runtime behavior. | Revert the Phase B review module/CLI/tests/onboarding docs; ignore generated `phoenix_phase_b_review.*` model-quality artifacts. No strategy, execution, broker, cron, allocation, or promotion behavior changes are in scope. |
| FR-053 Argo Phase B Regime Selection Validation | Investment Confidence / Research Evidence | `ACTIVE_RESEARCH` | LOW | Existing Argo selection artifacts, model tournament, promotion readiness, VIX/regime data, FR-069 Phase C onboarding | phase_c_research_onboarded | Validate Argo as a research-only regime overlay/model-selection layer, including stability, transition diagnostics, input freshness, no-lookahead checks, and the distinction between leaderboard winner and decision-grade recommendation. Argo is now represented as a governed FR-069 Research-stage meta-model candidate via `docs/governance/fr_active/fr_069_argo_onboarding_packet.md` and evidence template `docs/governance/fr_active/fr_069_argo_evidence_envelope_template.json`; this does not activate allocation switching or runtime behavior. | Revert the Phase B validation module/CLI/tests/onboarding docs; ignore generated `argo_phase_b_validation.*` model-quality artifacts. No capital routing or production selection behavior changes are in scope. |
...
| FR-068 PIT Universe + Polaris/Orion/Lyra Rebaseline | Research / Survivorship Remediation | `PHASES_1_3_COMPLETE` | LOW (research-only) | Sharadar (FR-067), PIT universe + caerus_large_cap family + SEP cache, Orion/Lyra audit packet | orion_lyra_pit_artifacts_pending | Phase 1 PIT universe (20,618 secs, 14,790 delisted) + `Universe(as_of_date)`; Phase 2 impact (SEVERE; 71.7% delisted); Phase 2.5 caerus_large_cap family (1,600; 354 delisted) + full SEP price hydration; Phase 3 Polaris priced rebaseline on the committed momentum harness = **MATERIAL** (Sharpe 1.054→0.851, MaxDD −43%→−54%; CAGR 28.83%→30.68%). Legacy = non-decision-grade; promotion evidence must carry `universe_method=pit_universe`. Orion/Lyra audit packet at `docs/governance/fr_active/fr_068_orion_lyra_pit_rebaseline_packet.md` finds no statistically meaningful Lyra lead in available matched evidence, but actual Orion/Lyra PIT rebaseline artifacts remain pending; DAILY-marketcap PIT family + index membership families are later phases. | Revert rebaseline research modules + artifacts and the Orion/Lyra audit packet if needed; PIT data is gitignored/regenerable. No execution, model, cron, registry, or holdout change in scope. |
...
6. Phoenix, Cassiopeia, Cygnus, and Argo remain Research-stage under FR-069
   onboarding packets; future sleeve work continues only after decision-grade
   evidence templates are proven.
