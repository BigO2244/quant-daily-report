...
3. FR-DH data hydration is the current read-only research-data foundation
   ...
   lineage/version manifests and coverage diagnostics before sleeve migration.
...
- **Current state:** FR-DH governance, catalog, hydration swarm, credential-safe
  ...
  feature-store scaffolding, validation, and internal `research_data` load APIs
  exist. Generated data artifacts remain ignored local research outputs. No
...
- **Recommended next implementation:** observe-only lineage/version manifests
  and coverage diagnostics. Keep remaining blocked datasets classified by
  source, credential, legal, or business-decision blocker, and close
