...
| Shadow performance series | `outputs/shadow_candidates/performance/shadow_nav_series.csv`, `shadow_summary.json`, `shadow_nav_restatement_manifest.json` | `derived`, `diagnostic` | Shadow tracking, refresh tooling, governed restatement tooling | Shadow CIO report, promotion audits | Canonical operational method is dated same-day close-to-close observation (`dated_same_day_close_to_close_v1`). Latest NAV date must align with evaluation date or carry stale reason. Legacy backtest-style CSV lineage is superseded for operational scorecards after the 2026-06-13 owner decision. | Rewritten only by governed refresh or explicit restatement with backup/manifest. | Retain incident backups and restatement manifests; long-term retention window not formalized. |
...
| Sleeve numeric trace artifacts | `outputs/runs/<RUN_ID>/audit/sleeve_numeric_trace_<sleeve_id>_<trade_date>.json` | `diagnostic`, `runtime` | Sleeve validity / daily report runtime | Operators, allocation cash-route review, target-attainment review | Expected when non-finite sleeve equity/strength/output invalidates a sleeve and a run context is available; reports reason codes, downstream effect, and first bad event when known. | One per invalid sleeve/run/trade date; may be overwritten by same run replay. | Retain with run-root incident evidence, especially when large allocation weight routes to CASH. |
