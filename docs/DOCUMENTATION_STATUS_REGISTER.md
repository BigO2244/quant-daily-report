...
| `docs/evidence/generic_lyra_v2_source_deployment_2026-08-19.json` | `CANONICAL_FOR_ITS_DEPLOYMENT` | Exact reviewed source `20743ad...` deployed cleanly with P0-1 implementation blockers closed; config and schedules remain disabled, both gates remain armed, and factual activation remains `NO_GO` pending the 2026-08-24/25 evidence and explicit owner approvals |
| `docs/evidence/governed_lyra_capture_boundary_source_deployment_2026-08-19.json` | `CANONICAL_FOR_CURRENT_CAPTURE_SOURCE_DEPLOYMENT` | Exact reviewed source `44987faf...` deployed cleanly with the one-date 2026-08-25 advisory capture boundary; capture config and cron remain absent, generic Live remains disabled and armed, and no broker call or authority change occurred |
| `docs/evidence/governed_lyra_capture_boundary_disabled_install_2026-08-19.json` | `CANONICAL_FOR_CURRENT_CAPTURE_INSTALLATION` | Exact disabled capture config and inert 2026-08-25 cron installed; flag remains `0`, disabled rehearsal proves no input read/write, PAPER and unrelated cron bytes are preserved, both Live gates remain armed, and no broker call or authority change occurred |
| `docs/governance/decision_records/generic_live_v1_dynamic_balance_owner_decision_20260824.json` | `CANONICAL_OWNER_POLICY_AUTHORITY` | Brett-approved cash-only dynamic Live policy superseding the exact eight fixed-$460 artifacts: no nominal ceiling, 95% of fresh broker net equity gross cap, buys limited by complete-history settled cash after subtracting T+1-unsettled sells and a 5% reserve, no buying power/margin/borrowing/unsettled funds; exact hash must be independently pinned and it grants no execution authority by itself |
| `docs/evidence/generic_live_v1_dynamic_monday_rehearsal_20260824.json` | `CANONICAL_SESSION_BLOCKER` | Requested Monday 2026-08-24 session is `BLOCKED_NO_TRADE_REARMED`: Monday precompute cannot use Monday-close weekly Lyra evidence, first canonical execution is Tuesday 2026-08-25, and no broker/schedule/kill/PAPER state changed |
| `docs/evidence/generic_live_dynamic_source_deployment_2026-08-19.json` | `CANONICAL_FOR_CURRENT_DYNAMIC_SOURCE_DEPLOYMENT` | Independently reviewed dynamic-balance source deployed at exact `9497bd51...`; VM 52/52 focused tests pass, config/crontab/env hashes are unchanged, gate is ARMED, all execution flags remain zero, and dynamic config installation remains fail-closed pending a reviewed current fee-schedule hash |
| `docs/governance/proposals/generic_live_v1_owner_decision_2026-08-24.pending.json` | `SUPERSEDED_PROPOSAL` | Preserved first draft; Monday morning cannot consume Monday's completed close, so this proposal must not be approved or reinterpreted for Tuesday execution |
| `docs/governance/proposals/generic_live_v1_owner_decision_2026-08-24.supersession.json` | `CANONICAL_FOR_PROPOSAL_STATUS` | Immutable supersession of the 2026-08-24 proposal before owner action; binds the corrected Tuesday proposal and grants no authority |
| `docs/governance/proposals/generic_live_v1_owner_decision_2026-08-25.pending.json` | `SUPERSEDED_PROPOSAL` | Preserved fixed-$460 Tuesday proposal; superseded by the later owner-approved dynamic-balance capital policy and incapable of authorizing activation |
| `docs/governance/proposals/lyra_governed_evidence_policy_v1.pending.json` | `RESEARCH_OR_PROPOSAL` | Exact pending owner-policy proposal for the Lyra risk, liquidity, capacity, calendar, and canonical full-L1 turnover evidence used by the prospective 2026-08-25 capture; grants no authority until Brett explicitly approves it |
