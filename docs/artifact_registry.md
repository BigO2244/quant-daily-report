...
| Sleeve numeric trace artifacts | Sleeve validity / daily report runtime | `outputs/runs/<RUN_ID>/audit/sleeve_numeric_trace_<sleeve_id>_<trade_date>.json` | Operators, allocation cash-route review, target-attainment review, governance | Expected when a sleeve is invalidated by non-finite terminal equity, strength, or numeric output and a run context is available; must identify sleeve id, trade date, reason code, downstream effect, and first bad event when known. | MEDIUM | MEDIUM when emitted from the same run root with reason codes and first-event metadata; LOW if trace is missing for an invalid sleeve or first-event metadata is unavailable. |
