...
scp \
  ...
  "${repo_root}/web/dashboard/quant_daily_performance.css" \
  "${repo_root}/web/dashboard/quant_daily_performance.js" \
...
# NOTE: This deploy does NOT sync credentials or research artifacts to the VM.
#   - Credentials: the refresh service reads the canonical repo .env
#     (/home/brettolson/quant-daily-report/.env), the same file the trading cron
#     uses. There is no separate ~/.caerus/alpaca.env to push anymore.
#   - Cockpit artifacts (sleeves, governance, evidence, runs, shadow, etc.) are
#     PRODUCED ON THE VM by its own cron pipeline. Pushing local copies from here
#     would overwrite fresh production data with stale dev copies.
