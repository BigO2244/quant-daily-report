...
def _write_execution_results(run_root: Path, payload: dict[str, object], paper_summary: dict[str, object]) -> Path:
    ...
    results = {
        ...
        "buy_phase_status": payload.get("buy_phase_status"),
        "buy_phase_completion_reason": payload.get("buy_phase_completion_reason"),
        "buy_fill_poll_count": payload.get("buy_fill_poll_count"),
        "buy_fill_observation_window_seconds": payload.get("buy_fill_observation_window_seconds"),
        "posttrade_snapshot_stage": payload.get("posttrade_snapshot_stage"),
        "posttrade_snapshot_timestamp": payload.get("posttrade_snapshot_timestamp"),
        "actual_posttrade_cash_source": payload.get("actual_posttrade_cash_source"),
        "submitted_buy_count": int(payload.get("submitted_buy_count") or 0),
        "filled_buy_count": int(payload.get("filled_buy_count") or 0),
        "pending_buy_count": int(payload.get("pending_buy_count") or 0),
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        for key in (
            ...
            "partial_buy_count",
            "observed_buy_orders",
            "posttrade_snapshot_stage",
            "posttrade_snapshot_timestamp",
            "actual_posttrade_cash_source",
