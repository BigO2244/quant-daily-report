...
from research_data.normalization import P2_DATASETS
...
def validate_p2_normalization_artifact(path: Path, *, repo_root: Path | None = None) -> list[str]:
    payload = read_json(path)
    root = Path(repo_root or REPO_ROOT)
    errors: list[str] = []

    if payload.get("schema_version") != "p2_normalization_manifest_v1":
        errors.append("p2 normalization schema_version must be p2_normalization_manifest_v1")
    if payload.get("runtime_impact") != "read_only_normalization_no_trading_path_changes":
        errors.append("p2 normalization runtime_impact must be read_only_normalization_no_trading_path_changes")
    for field in ("generated_at", "as_of_date", "dataset_count", "normalized_dataset_count", "failed_dataset_count"):
        if field not in payload:
            errors.append(f"p2 normalization manifest missing {field}")

    datasets = payload.get("datasets")
    if not isinstance(datasets, list):
        return errors + ["p2 normalization manifest missing datasets list"]
    ...
    for item in datasets:
        ...
        if dataset_id not in P2_DATASETS:
            errors.append(f"{dataset_id}: unsupported P2 dataset")
    ...
    if duplicate_count:
        errors.append(f"p2 normalization manifest contains {duplicate_count} duplicate dataset ids")
    return errors
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate P2 normalized research data artifacts.")
    parser.add_argument("--path", type=Path, default=REPO_ROOT / "data" / "manifests" / "p2_normalization_manifest.json")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_p2_normalization_artifact(args.path, repo_root=args.repo_root)
