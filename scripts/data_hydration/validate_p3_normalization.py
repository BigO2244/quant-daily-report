...
from research_data.normalization import P3_DATASETS
...
def validate_p3_normalization_artifact(path: Path, *, repo_root: Path | None = None) -> list[str]:
    payload = read_json(path)
    root = Path(repo_root or REPO_ROOT)
    errors: list[str] = []

    if payload.get("schema_version") != "p3_normalization_manifest_v1":
        errors.append("p3 normalization schema_version must be p3_normalization_manifest_v1")
    if payload.get("runtime_impact") != "read_only_normalization_no_trading_path_changes":
        errors.append("p3 normalization runtime_impact must be read_only_normalization_no_trading_path_changes")
    datasets = payload.get("datasets")
    if not isinstance(datasets, list):
        return errors + ["p3 normalization manifest missing datasets list"]
    ...
    for item in datasets:
        ...
        if dataset_id not in P3_DATASETS:
            errors.append(f"{dataset_id}: unsupported P3 dataset")
    ...
    if duplicate_count:
        errors.append(f"p3 normalization manifest contains {duplicate_count} duplicate dataset ids")
    return errors
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate P3 normalized research data artifacts.")
    parser.add_argument("--path", type=Path, default=REPO_ROOT / "data" / "manifests" / "p3_normalization_manifest.json")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_p3_normalization_artifact(args.path, repo_root=args.repo_root)
