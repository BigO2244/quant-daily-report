...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--order-lifecycle", type=Path)
    parser.add_argument("--broker-orders", type=Path)
    parser.add_argument("--broker-fills", type=Path)
    parser.add_argument("--ending-state", type=Path)
    parser.add_argument("--collect-from-broker", action="store_true")
    parser.add_argument("--broker-evidence-directory", type=Path)
    parser.add_argument("--published-pointer-path", type=Path)
    ...
    try:
        ...
        if args.collect_from_broker:
            if args.broker_evidence_directory is None or args.published_pointer_path is None:
                raise RuntimeError(
                    "broker collection requires evidence directory and pointer path"
                )
            secure_path(
                args.broker_evidence_directory, allowed_roots=[args.state_root],
                must_exist=False, kind="directory",
            )
            secure_path(
                args.published_pointer_path, allowed_roots=[args.state_root],
                must_exist=False, kind="file",
            )
        elif any(
            path is None for path in (
                args.order_lifecycle, args.broker_orders,
                args.broker_fills, args.ending_state,
            )
        ):
            raise RuntimeError(
                "explicit raw posttrade mode requires lifecycle/order/fill/ending paths"
            )
        secure_path(
        ...
        if not args.collect_from_broker:
            input_specs[2:2] = [
        ...
        for label, path, trigger in input_specs:
            ...
            read_roots = (
                [args.input_root, args.state_root]
                if args.collect_from_broker and label == "submission"
                else [args.input_root]
            )
            secure_path(path, allowed_roots=read_roots, must_exist=True, kind="file")
            loaded[label] = secure_read_json(path, allowed_roots=read_roots)
            if label == "submission":
        ...
        common = {
            ...
            "deployment_state": loaded["deployment_state"],
        ...
        if args.collect_from_broker:
            ...
            built = collect_and_finalize_generic_live_v1_posttrade(
                broker=AlpacaBroker.from_env(), observed_at=args.finalized_at,
                evidence_directory=args.broker_evidence_directory,
                published_pointer_path=args.published_pointer_path,
                **common,
            )
        else:
            failure_trigger = "RECONCILIATION_BREAK"
            ...
            built = build_and_finalize_generic_live_v1_production_posttrade(
                ...
                ending_state=loaded["ending_state"], **common,
            )
    except Exception:
