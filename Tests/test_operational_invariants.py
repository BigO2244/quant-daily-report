...
def test_market_closed_day_with_planned_payload_is_expected_skip(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.chdir(tmp_path)
    ...
    assert invariant["reason_code"] == "MARKET_CLOSED_DAY"
    assert _result(report, "terminal_status_requires_reason")["status"] == "PASS"
    assert _result(report, "required_precompute_artifacts")["status"] == "PASS"
