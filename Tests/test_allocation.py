...
def test_trend_only_100pct():
    ...
    assert (
        abs(trend_alloc - 1.0) < WEIGHT_TOLERANCE
    ), f"Trend should be 100%, got {trend_alloc:.1%}"
    assert (
        abs(val_alloc - 0.0) < WEIGHT_TOLERANCE
    ), f"Valuation should be 0%, got {val_alloc:.1%}"
...
def test_both_active_equal_strength():
    ...
    trend = create_sleeve_output(
        [{"ticker": "AAPL", "target_weight": 1.0}], "sleeve_trend", strength=1.0
    )
    val = create_sleeve_output(
        [{"ticker": "IBM", "target_weight": 1.0}], "sleeve_2", strength=1.0
    )
    ...
    assert (
        abs(trend_alloc - 0.5) < WEIGHT_TOLERANCE
    ), f"Trend should be 50%, got {trend_alloc:.1%}"
    assert (
        abs(val_alloc - 0.5) < WEIGHT_TOLERANCE
    ), f"Valuation should be 50%, got {val_alloc:.1%}"
...
def test_no_sleeves_100_cash():
    ...
    assert (
        abs(cash_weight - 1.0) < WEIGHT_TOLERANCE
    ), f"CASH should be 100%, got {cash_weight:.1%}"
...
def test_weights_sum_to_1():
    ...
    scenarios = [
        ...
        (
            [{"ticker": "AAPL", "target_weight": 0.5}],
            [{"ticker": "IBM", "target_weight": 0.5}],
            1.0,
            1.0,
        ),
        (
            [{"ticker": "AAPL", "target_weight": 0.3}],
            [{"ticker": "IBM", "target_weight": 0.7}],
            0.8,
            0.2,
        ),
        ([], [], 0.0, 0.0),  # All cash
    ...
    for i, (trend_pos, val_pos, trend_str, val_str) in enumerate(scenarios):
        ...
        assert (
            abs(total - 1.0) < WEIGHT_TOLERANCE
        ), f"Scenario {i+1}: weights should sum to 1.0, got {total:.6f}"
...
def test_position_cap_no_renormalize():
    ...
    aapl_weight = result.combined_weights[result.combined_weights["ticker"] == "AAPL"][
        "target_weight"
    ].iloc[0]
    assert (
        abs(aapl_weight - 0.10) < WEIGHT_TOLERANCE
    ), f"AAPL should be capped at 10%, got {aapl_weight:.1%}"

    # Remaining 90% should be CASH
    cash_weight = result.cash_weight
    assert (
        abs(cash_weight - 0.90) < WEIGHT_TOLERANCE
    ), f"CASH should be 90%, got {cash_weight:.1%}"

    # Should have recorded the skip
    assert (
        len(result.skipped_trades) > 0
    ), "Should have recorded skipped trade for cap hit"
...
def test_strength_proportional_allocation():
    ...
    assert (
        abs(trend_alloc - 0.75) < WEIGHT_TOLERANCE
    ), f"Trend should be 75%, got {trend_alloc:.1%}"
    assert (
        abs(val_alloc - 0.25) < WEIGHT_TOLERANCE
    ), f"Valuation should be 25%, got {val_alloc:.1%}"
