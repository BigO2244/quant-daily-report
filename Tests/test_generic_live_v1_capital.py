...
def test_limit_price_fee_and_dynamic_95_5_limits_are_explicit() -> None:
    ...
    proof = build_generic_live_v1_capital_proof(
        exact_plan=plan, fresh_equity_usd=460.9,
        fresh_cash_usd=plan["starting_cash"],
    )
    ...
    assert proof["worst_case_posttrade_gross_usd"] == 0.0
...
def test_lower_fresh_equity_tightens_cap_while_sell_remains_de_risking() -> None:
    _, plan = _ready()
    proof = build_generic_live_v1_capital_proof(
        exact_plan=plan, fresh_equity_usd=400.0,
        fresh_cash_usd=plan["starting_cash"],
    )
