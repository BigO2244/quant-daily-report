...
def _build_email_summary(operator_summary: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    status = (
        ...
        else ("EMAIL_LOOP_WARNING" if emails_sent > 3 else "INCOMPLETE")
    )
...
def _build_broker_context(
    run_root: Path, operator_summary: Optional[Dict[str, Any]]
) -> Dict[str, Any]:
    ...
    preflight = (
        summarize_pretrade_broker_policy({"account": pretrade_account_obj})
        if pretrade_account_obj
        else {}
    )

    return {
        "broker_authoritative_state": bool(
            (operator_summary or {}).get("broker_authoritative_state")
        ),
        "broker_pretrade_snapshot_ok": (operator_summary or {}).get(
            "broker_pretrade_snapshot_ok"
        ),
        "broker_posttrade_snapshot_ok": (operator_summary or {}).get(
            "broker_posttrade_snapshot_ok"
        ),
        "duplicate_guard_status": (operator_summary or {}).get("duplicate_guard_status"),
        "broker_preflight_status": (operator_summary or {}).get(
            "broker_preflight_status"
        )
        or preflight.get("broker_preflight_status"),
        "broker_preflight_account_status": (operator_summary or {}).get(
            "broker_preflight_account_status"
        )
        or preflight.get("broker_preflight_account_status"),
        "broker_preflight_cash": (operator_summary or {}).get(
            "broker_preflight_cash"
        )
        if (operator_summary or {}).get("broker_preflight_cash") is not None
        else preflight.get("broker_preflight_cash"),
        "broker_preflight_equity": (operator_summary or {}).get(
            "broker_preflight_equity"
        )
        if (operator_summary or {}).get("broker_preflight_equity") is not None
        else preflight.get("broker_preflight_equity"),
        "broker_preflight_buying_power": (operator_summary or {}).get(
            "broker_preflight_buying_power"
        )
        if (operator_summary or {}).get("broker_preflight_buying_power") is not None
        else preflight.get("broker_preflight_buying_power"),
        "broker_preflight_restriction_flags": dict(
            (operator_summary or {}).get("broker_preflight_restriction_flags")
            or preflight.get("broker_preflight_restriction_flags")
            or {}
        ),
        "broker_preflight_warning_flags": list(
            (operator_summary or {}).get("broker_preflight_warning_flags")
            or preflight.get("broker_preflight_warning_flags")
            or []
        ),
        "broker_pdt_risk_status": (operator_summary or {}).get("broker_pdt_risk_status")
        or preflight.get("broker_pdt_risk_status"),
        ...
        "broker_pdt_daytrading_buying_power": (
            ...
            if (operator_summary or {}).get("broker_pdt_daytrading_buying_power")
            is not None
            else preflight.get("broker_pdt_daytrading_buying_power")
        ),
        "broker_pdt_flags": list(
            (operator_summary or {}).get("broker_pdt_flags")
            or preflight.get("broker_pdt_flags")
            or []
        ),
        "broker_pdt_warning_message": (operator_summary or {}).get(
            "broker_pdt_warning_message"
        )
        or preflight.get("broker_pdt_warning_message"),
        ...
        "post_execution_recon_status": (operator_summary or {}).get(
        ...
        "broker_buying_power_at_planning": (
            ...
            if (operator_summary or {}).get("broker_buying_power_at_planning")
            is not None
            else preflight.get("broker_preflight_buying_power")
        ),
        "reserve_cash_policy": dict(
            (operator_summary or {}).get("reserve_cash_policy") or {}
        ),
        "expected_sell_proceeds": (operator_summary or {}).get(
            "expected_sell_proceeds"
        ),
        "expected_sell_proceeds_conservative": (operator_summary or {}).get(
            "expected_sell_proceeds_conservative"
        ),
        "requested_buy_notional": (operator_summary or {}).get(
            "requested_buy_notional"
        ),
        "allowed_buy_notional": (operator_summary or {}).get("allowed_buy_notional"),
        "capital_constraint_triggered": (operator_summary or {}).get(
            "capital_constraint_triggered"
        ),
        "clipped_or_deferred_buys_count": int(
            (operator_summary or {}).get("clipped_or_deferred_buys_count") or 0
        ),
        "affected_symbols": list((operator_summary or {}).get("affected_symbols") or []),
        "repair_suggestions": list(
            (operator_summary or {}).get("repair_suggestions") or []
        ),
        "duplicate_fill_suspicions_count": int(
            (operator_summary or {}).get("duplicate_fill_suspicions_count") or 0
        ),
        ...
        "pretrade_equity": pretrade_account_obj.get("equity")
        or pretrade_account_obj.get("portfolio_value"),
