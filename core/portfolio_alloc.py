...
DEFAULT_MAX_POSITION_PCT = float(os.getenv("MAX_POSITION_PCT", "0.15"))
DEFAULT_MAX_SLEEVE_EXPOSURE = 1.00
DEFAULT_MAX_TURNOVER = 0.50
DEFAULT_MIN_GROSS_EXPOSURE_NORMAL = float(os.getenv("MIN_GROSS_EXPOSURE_NORMAL", "0.95"))
DEFAULT_NORMAL_MAX_CASH_PCT = float(os.getenv("NORMAL_MAX_CASH_PCT", "0.05"))
...
class PortfolioAllocator:
    ...
    def __init__(
        ...
        min_gross_exposure_normal: float = DEFAULT_MIN_GROSS_EXPOSURE_NORMAL,
        normal_max_cash_pct: float = DEFAULT_NORMAL_MAX_CASH_PCT,
        ...
        self.min_gross_exposure_normal = max(0.0, min(1.0, float(min_gross_exposure_normal)))
        self.normal_max_cash_pct = max(0.0, min(1.0, float(normal_max_cash_pct)))
    ...
    def _compute_sleeve_allocations(self, sleeve_outputs: list[SleeveOutput]) -> dict[str, float]:
        ...
        if not self.risk_off:
            scale = max(0.0, 1.0 - self.normal_max_cash_pct)
            allocations = {name: weight * scale for name, weight in allocations.items()}
    ...
    def _boost_to_min_gross_exposure(self, combined: pd.DataFrame) -> pd.DataFrame:
        ...
        if gross <= WEIGHT_TOLERANCE or gross >= self.min_gross_exposure_normal - WEIGHT_TOLERANCE:
            return df

        if self.min_gross_exposure_normal <= 0:
            return df
        ...
        needed = min(self.min_gross_exposure_normal - gross, total_headroom)
